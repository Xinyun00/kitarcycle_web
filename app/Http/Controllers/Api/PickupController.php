...
class PickupController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'image' => 'required|image|mimes:jpeg,png,jpg,gif|max:2048', // Assuming image is required
            'driver_id' => 'required|exists:drivers,id',
            'schedule_id' => 'required|exists:driver_schedules,id,status,available', // Ensure schedule is available
        ]);
        ...
        try {
            ...
            // This will store the image in 'storage/app/public/kitarcycle/pickups'.
            if ($request->hasFile('image')) {
                $imagePath = $request->file('image')->store('kitarcycle/pickups', 'public');
            }
            ...
            $pickup = Pickup::create([
                ...
                'images' => $imagePath, // Save the path returned by store().
            ]);
            ...
            // CHANGE: If an image was saved during a failed transaction, delete it.
            if ($imagePath) {
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $pickupData = $request->only(['category_id', 'address', 'estimated_weight']);
            ...
            if ($request->hasFile('image')) {
                ...
                // Store the new image in the correct 'pickups' directory.
                $newImagePath = $request->file('image')->store('kitarcycle/pickups', 'public');
                $pickupData['images'] = $newImagePath;
            }

            // Validate schedule-driver relationship if both are being updated
            if ($request->has(['driver_id', 'schedule_id'])) {
                $schedule = DriverSchedule::where('id', $request->schedule_id)
                    ->where('driver_id', $request->driver_id)
                    ->where('status', 'available')
                    ->first();

                if (!$schedule) {
                    return response()->json(['message' => 'Invalid or unavailable schedule.'], 422);
                }

                // Free old schedule
                DriverSchedule::where('id', $pickup->schedule_id)->update(['status' => 'available']);
                // Mark new one as booked
                $schedule->update(['status' => 'booked']);

                $pickupData['schedule_id'] = $request->schedule_id;
            ...
            Log::error('Error updating pickup: ' . $e->getMessage());
